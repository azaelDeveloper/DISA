...
function setLicense(){
	...
	var urlS = "http://siaa.mx/App/appdataservice.asmx/SIAA?ID=" + license;	
	if (license != null){
...
function sellersViewModel(sellers){	
	...
	var query = "INSERT INTO sellers (sellerIDERP, nameSeller, license, active, routes, timestamp) VALUES (" + self.sellers()[0].sellerIDERP + ", '"+ self.sellers()[0].nameSeller +"', '"+ self.sellers()[0].license + "', '"+ self.sellers()[0].active +"', '"+ self.sellers()[0].routes +"',  '" + getCurrentDateTime() + "')";
	tx.executeSql(query);

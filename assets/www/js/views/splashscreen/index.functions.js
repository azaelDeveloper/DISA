...
function populateDB(tx) {		
	...
	tx.executeSql('CREATE TABLE IF NOT EXISTS contract(id INTEGER PRIMARY KEY, accepted NUMERIC, account_type INTEGER, timestampp NUMERIC)');	
	// SELLERS
	...
	tx.executeSql('CREATE TABLE IF NOT EXISTS commercial_invoice (id INTEGER PRIMARY KEY, clientID TEXT, nameClient TEXT, num_Fact REAL, amount REAL, paid REAL, expiration TEXT, timestampp NUMERIC)');	
	//COMERCIAL_DETAIL
	//tx.executeSql('DROP TABLE IF EXISTS commercial_detail');	
	tx.executeSql('CREATE TABLE IF NOT EXISTS commercial_detail (id INTEGER PRIMARY KEY, num_Fact TEXT, paid REAL, totalPaid REAL, timestampp NUMERIC)');	
	//PAYMENTS
	/* Check this */
	//tx.executeSql('DROP TABLE IF EXISTS payments');
	tx.executeSql('CREATE TABLE IF NOT EXISTS payments (id INTEGER PRIMARY KEY, IdClient INTEGER, nameClient TEXT, num_Fact REAL, amount TEXT,  timestampp NUMERIC)');
	// DEBTS
	...
	tx.executeSql('CREATE TABLE IF NOT EXISTS categorys (id INTEGER PRIMARY KEY, category TEXT, timestampp NUMERIC)');
	//SELLS
	//tx.executeSql('DROP TABLE IF EXISTS sells');
	tx.executeSql('CREATE TABLE IF NOT EXISTS sells (id INTEGER PRIMARY KEY, idClient TEXT, nameClient TEXT, total REAL, idSeller TEXT, synchronized NUMERIC, timestampp NUMERIC)');
	//SELLSDETAIL
	//tx.executeSql('DROP TABLE IF EXISTS sellsdetail');
	tx.executeSql('CREATE TABLE IF NOT EXISTS sellsdetail (id INTEGER PRIMARY KEY, idSell TEXT, idProduct TEXT, unit TEXT, category TEXT, nameProduct TEXT, quantity INTEGER, price REAL, priceType TEXT, minPrice REAL, iva REAL, ieps REAL, subtotal REAL)');
}
...
function foundAgreement(tx, results) {
	...
	if (len == 0) {
		getGPS();
		var query = "INSERT INTO contract(accepted, timestampp) VALUES (0, '" + getCurrentDateTime() + "')";
		tx.executeSql(query);
		getPage("file:///android_asset/www/views/initial_configuration/contract.title.html", "file:///android_asset/www/views/initial_configuration/contract.html");
	}
}
